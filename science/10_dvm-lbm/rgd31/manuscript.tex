...
%\section{CONCLUSIONS}
%
%In the present paper we have introduced the hybrid method of solving of the DV models for the BGK kinetic model and LB equations.
%Non-equilibrium spatial zones are modeled using general DV (or finite-difference schemes) for the BGK model and the spatial domains
%where the distribution function is close to equilibrium are considered using LB models.
%At the overlapping domain for these two methods we have introduced the matching between the solutions.
%The hybrid method shows good precision, moreover significant computation speed-up for the hybrid method over the DV model for the BGK equation.
