...
\begin{equation}\label{eq:diffuse}
    f(t,\bxi) = \equil{f} \br{ \bxi;
        -2\sqrt\frac\pi{T_B} \int_{\bxi'\bn<0} \bxi'\bn f(t,\bxi')\dxi', \bv_B, T_B
    } \quad (\bxi\bn>0),
\end{equation}
...
\end{equation}
It is convenient to deal with weighted values \(\hat{f}_j = w_j f_j\).
The evolution of \(\hat{f}_j\) is governed by the system of partial differential equations
\begin{equation}\label{eq:dvm}
    \pder[\hat{f}_j]{t} + \bxi_j\pder[\hat{f}_j]{\bx} = \frac1kJ(\hat{f}_j)
\end{equation}
...
\begin{equation}\label{eq:equilibrium-bgk}
    \equil{f}_{\DV,j} = \exp\br{\sum_{r=1}^5\beta_r\psi_{jr}}, \quad
    \psi_{jr} = \transpose{\br{1,\bxi_j,|\bxi|^2}},\end{equation}
...
% \begin{equation}\label{eq:dvm}
% \pder[f_j]{t}+ \bxi_j\pder[f_j]{\bx}=\frac{1}{Kn}(f^{(eq)}_j-f_j), \quad j=1 \ldots M,
% \end{equation}
...
% \end{equation}
% The equation \eqref{eq:beta} is solved numerically using Newton algorithm ~\cite{Mieussens2000}.
% The construction \eqref{eq:equilibrium-bgk}-\eqref{eq:beta} guarantees
% that the equilibrium state maximizes entropy and the conservation laws are satisfied.
...
\begin{equation}\label{eq:first_flux}
    f^{n+1/2}_{1/2}(\bxi_j) = \displaystyle\frac{\sum_{\bxi'_j\bn>0}f^{n+1/2}_{1/2}(\bxi'_j)}
...
The authors are grateful to Victor Ambruș, whose comments helped to improve the final version of the manuscript.
