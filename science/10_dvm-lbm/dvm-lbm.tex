...
\begin{equation}\label{eq:Boltzmann}
    \pder[f]{t} + \bxi\bdot\pder[f]{\bx} = \frac{\sqrt2}k J(f),
\end{equation}
...
\end{equation}
The Knudsen number \(k=\ell_0\sqrt\pi/2L\), where \(\ell_0\) is the mean free path,
can be expressed in terms of the reference gas viscosity \(\refer\mu\)~\cite{Cercignani2000}:
\begin{equation}\label{eq:Knudsen_number}
    k = \frac{\sqrt2\refer\mu\refer{c}}{\refer{p}L}.
\end{equation}
...
\begin{equation}\label{eq:diffuse}
    f(t,\bx_B,\bxi) = \equil{f} \br{ \bxi;
        -\sqrt\frac{2\pi}{T_B} \int_{\bxi'\bdot\bn<0} \bxi'\bdot\bn f(t,\bx_B,\bxi')\dxi', \bv_B, T_B
    } \quad (\bxi\bdot\bn>0),
\end{equation}
...
Within the DV framework, the admissible particle velocities are restricted to set \(\Set{\bxi_j}{j=1,\dots,N}\).
Under this assumption, an arbitrary moment \(\phi(\bxi)\) of \(f\) is approximated as
...
\begin{equation}\label{eq:dvm}
    \pder[f_j]{t} + \bxi_j\bdot\pder[f_j]{\bx} = \frac1k J_N(f_j)
\end{equation}
...
which is consistent with \(J(f)\) when \(N\) goes to infinity~\cite{Aristov2001}.
...
\begin{equation}\label{eq:equilibrium-bgk}
    \equil{f}_{\DV,j}(\hat\bm) = \frac{\hat\rho w_j}{(2\pi \hat{T})^{3/2}}
        \exp\br{-\frac{|\bxi_j-\hat\bv|^2}{2\hat{T}}}, \quad
...
\begin{equation}\label{eq:equilibrium-bgk-moments}
    \sum_j \bpsi_j \equil{f}_{\DV,j}(\hat\bm) = \rho\transpose{\br{1, \bv, |\bv|^2 + 3T}}, \quad
...
\begin{equation}\label{eq:equilibrium-bgk-aniso}
    \equil{f}_{\DV,j}(\hat\bm) = \hat{\rho} w_j \prod_\alpha\br{ 2\pi \hat{T}_\alpha }^{-1/2}
        \exp\br{-\sum_\alpha\frac{\br{\xiaj-\hat{v}_\alpha}^2}{2\hat{T}_\alpha}}, \quad
    \hat\bm = \transpose{\br{\hat\rho, \hat\bv, \hat{\bT}}} \in \mathbb{R}^7,
\end{equation}
where \(\xiaj \equiv \bxi_j\), vector \(\hat\bm\) is the solution of
...
Moreover, we assume that the particle can travel with the velocities $\bc_j, j=1 \ldots N$
...
\begin{equation}\label{eq:lbgk}
    \equil{f}_{\LB,j}(\bm) = \rho w_j\left( 1
...
\end{equation}
where $\bc_j$ are the lattice velocities, $c_s$ is the isothermal sound velocity defined by $\sum_jw_j\bc^2_j=3c_s^2$,
...
\begin{equation}
    \begin{gathered}
        ...
        \ai(x)=\sum_{m=1}^Mf_\DV(x,\bxi_m)\Hi(\bxi_m), \\
...
%    \equil{f}_\alpha = \rho\Bigg\{ 1 + 2\xiaj v_i + %\overbrace{
%        2\br{\xiaj v_i}^2 - v_i^2 + (T-1)\br{\xiaj^2 - %\frac{D}2}
%    }^\text{second order} + \\ \underbrace{
%        \frac{2\xibj v_j}3\left[ 2\br{\xiaj v_i}^2 - 3v_i^2 %+ 3(T-1)\br{\xiaj^2 - 1 - \frac{D}2}\right]
...
\begin{equation}\label{eq:vdf_fv}
    f^n_m(\bxi_j) = f\br{
        ...
        x = x_m \equiv \frac{\Delta{x_m}}2 + \sum_{k=1}^{m-1}\Delta{x_k}, \;
        \bxi = \bxi_j
    }
\end{equation}
...
is described using the BGK equation, while the LBGK model is employed for the internal zone.
For the BGK model of the Boltzmann equation, the plane Couette-flow problem can be reduced
to a one-dimensional integral equation, which has been solved very accurately in~\cite{Luo2015, Luo2016}.
...
This heat flux is caused by the diffuse-reflection boundary conditions and has non-polar singularity on $k$ at the boundary.
...
which results in better reproduction of the diffuse-reflection boundary conditions.
...
For instance, quantities \(E_p=\|f-f^{\mathrm{NSF}}\|_p/\|f\|_p\),
...
\begin{gather}
    ...
    f^{\mathrm{NSF}}_{\LB,j} = \equil{f}_{\LB,j} + w_j\xiaj\br{
...
\end{equation}
Taking into account the Courant--Friedrichs--Lewy (CFL) condition \(\gamma\leq1\)
...
Note that inequalities~\eqref{eq:fv:tvd} are often replaced by stronger ones, independent of \(\gamma\),
