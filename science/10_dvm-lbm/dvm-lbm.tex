...
i.e., the Mach number \(\Ma\) is close to zero and \(T = 1 + \OO{\Ma^2}\).
...
%%% Discretization of the proposed mapping
Discretization of~\eqref{eq:grad} and~\eqref{eq:a_coeffs} in the velocity space is pretty straightforward.
Particularly, the expansion coefficients~\eqref{eq:a_coeffs} are calculated as quadratures~\eqref{eq:cubature},
...
The described mapping method is natural for the Gauss--Hermite LB models.
...
\begin{equation}\label{eq:first_flux}
    f^{n+1/2}_{1/2}(\bxi_j) = \displaystyle\frac{\sum_{\xi_k>0}f^{n+1/2}_{1/2}(\bxi_k)}
        {\sum_{\xi_k<0}\xi_k\equil{f}_{\DV,k}(\bm_B)}
        \equil{f}_{\DV,j}(\bm_B), \quad \bm_B = \transpose{\br{1, \bv_B, T_B}},
\end{equation}
