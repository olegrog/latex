...
\begin{equation}\label{eq:vgrid_hermite}
    \xi_{\alpha,j} = x_{\Herm j} R, \quad w_{\alpha,j} = w_{\Herm j}\exp\br{x_{\Herm j}^2}R, \quad
...
\begin{equation}\label{eq:vgrid_polynomial}
    w_{y,j} = w_{y,1} + (j-1)^p \br{\xi^\cut - N_y w_{y,1}} \br{\sum_{i=1}^{N_y} (i-1)^p}^{-1}, \quad
...
The quarature weight in \(\mathbb{R}^3\) of discrete velocity \(\bxi_j \equiv \xi_{\alpha,j}\)
