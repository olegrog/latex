...
The third-order expansion in $\bv$ at unit temperature yields the following local equilibrium LB state:
\begin{equation}\label{eq:lbgk}
    \equil{f}_{\LB,j}(\bm) = \rho w_j\left( 1
        + \bxi_j\bdot\bv
        + \frac12\left[ (\bxi_j\bdot\bv)^2 - v^2 \right]
        + \frac{\bxi_j\bdot\bv}6\left[ (\bxi_j\bdot\bv)^2 - 3v^2 \right]
    \right), \quad j = 1,\ldots,N_\LB,
\end{equation}
where the discrete velocities \(\bxi_j\) and the corresponding weights \(w_j\) satisfy the following relations:
\begin{equation}
    \sum_j w_j = 1, \quad \sum_j w_j \bxi_j = 0, \quad \sum_j w_j \xi_j^2 = 3,
\end{equation}
