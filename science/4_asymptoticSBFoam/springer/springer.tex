...
We avoid to use \(\alpha < 1\) in favor of negative \(\alpha\)
for antisymmetric case \(T_1 = -\alpha\) and \(T_2 = 1\).
...
\(F_{i2} = -p_{ij2}n_j\), acting on the unit area of a body,
arises in the gas (Fig.~\ref{fig:forces:total},~\ref{fig:forces:inverse}).
To evaluate the total force acting on the inner cylinder, we imply that its length is equal to unity.
...
For \(\alpha=-5\), the distances \(d=0.69\) and \(d=0.94\) correspond to
the single stable equilibrium states for cylinders and spheres respectively.
The temperature gradient is included in the thermal stress force~\eqref{eq:force} as a cubic term.
The same dependence is shown for the force acting on bodies (Fig.~\ref{fig:force:alpha}).
...
\begin{figure}[ht]
    ...
    \begin{minipage}{.48\textwidth}
        ...
        \caption{The total force acting on the inner cylinder
        	versus the temperature ratio of the cylinders~\(\alpha\) for \(d=0.5\).
        	The dashed line corresponds to the cubic relation.}
        \label{fig:force:alpha}
    \end{minipage}
    \quad
    \begin{minipage}{.48\textwidth}
        \centering
...
The contribution of each term to the total value is clearly demonstrated
...
\begin{figure}[ht]
    ...
    \begin{minipage}{.48\textwidth}
        ...
        \caption{The maximum magnitude of \(u_{i1}\) versus
        	the temperature ratio of the cylinders \(\alpha\) for \(\beta=\pi/2\).
            	The dashed line corresponds to the cubic relation.}
        \label{fig:maxU}
    \end{minipage}
    \quad
...
The maximum magnitude of \(u_{i1}\) in dependence of \(\alpha\) is shown in Fig.~\ref{fig:maxU}.
For small \((\alpha-1)\), nonlinear thermal-stress flow diminished as a cubic function of temperature variations.
...
\( \delta T = T_\mathrm{AT} - T_\mathrm{HC} \)
along the major axis of the outer cylinder for \(\beta=\pi/2\).
