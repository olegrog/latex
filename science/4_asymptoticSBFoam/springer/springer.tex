...
\begin{abstract}
	Classical fluid dynamics describes the behavior of gas in terms of
	the Navier--Stokes equations. However, a rigorous asymptotic
	analysis of the Boltzmann equation for small Knudsen numbers
	restricts the scope of its application and leads to more complicated
	sets of differential equations in the general case. The present paper
	deals with the one which is valid for significant temperature
	variations and finite Reynolds numbers. A special-purpose solver on
	the open-source CFD platform OpenFOAM\textregistered{} is proposed
	for computer simulation of a slightly rarefied gas in an arbitrary
	geometry. Several temperature driven flows are considered as examples.
	\keywords{
		OpenFOAM \and
...
both have a periodic distribution of the temperature
\begin{equation}
	T_B = 1-\alpha\cos(2\pi x).
\end{equation}
The upper plate is at rest, the lower plate moves with the small velocity
\begin{equation}
	u_{Bi} = (\beta\Kn,0,0).
\end{equation}
...
The resulting temperature field (Fig.~\ref{fig:moving:T_asym}) is shown
...
The resulting velocity field \(u_{i1}\) is shown in Fig.~\ref{fig:moving:fluid} and Fig.~\ref{fig:moving:kn001}.
...
In the continuum limit, the velocity field tends to zero~\eqref{eq:Mach_constraint},
however \(u_{i1}\) remains finite and transforms the temperature field by means of~\eqref{eq:asymptotic3}.
Such an asymptotic behavior of a system of differential equations is called the \emph{ghost effect}.
Moreover, it is important to note that the asymptotic solution is very sensitive to the mutual velocity of the plates:
...
and is described first in~\cite{Kogan1971} with the Chapman--Enskog expansion
that give the similar equations to~\eqref{eq:asymptotic1}--\eqref{eq:asymptotic3} but
with slightly different transport coefficients.
...
Some experimental studies of the nonlinear thermal-stress flow can be found in~\cite{ExperimentsNTFS}.
...
A gas between nonconcentric spheres is also simulated in the same geometry~\ref{fig:spheres}.
