...
\usepackage[
    ...
    movenames=false,    % do not move authors behind the title
    sorting=none,
    url=false,
    doi=false,
    eprint=true,
    firstinits,         % initials instead of name
]{biblatex}
...
\author{О.~А.~Рогозин\thanks{oleg.rogozin@phystech.edu, Moсковский физико-технический институт}}
\date{}
...
\begin{abstract}
    ...
    асимптотического анализа кнудсеновского слоя на основе уравнения Больцмана.
...
Для малых чисел Кнудсена и особенно медленных течений, классический метод DSMC крайне трудоёмок.
Множество специализированных стохастических схем предложено в последние годы~\cite{Wagner2005, Hadji2013}.
Детерминистические методы решения уравнения Больцмана также весьма перспективны~\cite{Dimarco2014, Mieussens2014}.
Однако вычислительные трудности не были преодолены полностью до настоящего времени.
...
обсуждаются в~\cite{Levermore2012, Tan2016}.
...
тогда и только тогда, когда граничные значения \(T_{H1}\) и \(u_{jH1}\deltann{i}{j}\)
...
и граничных значений \(T_{H2}\) и \(u_{jH2}\deltann{i}{j}\).
...
выполнение H-теоремы Больцмана в дискретной форме~\cite{Dodulad2013}.
...
Решётчатое правило Коробова~\cite{Korobov1959, Sloan1994} используется в качестве
...
для анализа газокинетических процессов~\cite{Kloss2010, Kloss2011, Kloss2012}.
...
Кнудсена \(0.1\le\Kn\le5\) в~\cite{Sone1998}, исследована зависимость момента сил, действующих
на цилиндры, от угла поворота \(\beta\) на основе уравнений КГФ в~\cite{Rogozin2014}.
...
Зависимость режима течения от числа Кнудсена при \(\Kn<0.1\) не изучена в литературе,
картины конкурирующих течений также не были представлены до настоящего времени.
В частности, достижение приемлемого отношения сигнала к шуму при статистическом моделировании
...
Использование граничных условий второго порядка (рис.~\ref{fig:elliptic:flow-asym}) практически не меняет
качественную картину течения по сравнению с граничными условиями ведущего порядка (рис.~\ref{fig:elliptic:flow-kgf}).
Следует отметить только общее понижение модуля скорости из-за скачка скорости,
а также термострессового скольжения второго порядка.
...
Учёт членов следующего порядка в граничных условиях к уравнениям КГФ даёт малую поправку,
поэтому может быть рекомендован лишь для высокоточного численного анализа.
...
В настоящее время они известны лишь для простейшего потенциала твёрдых сфер и некоторых модельных уравнений~\cite{Takata2016},
что ограничивает применение в прикладных задачах.
