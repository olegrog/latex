...
\title{Численный анализ медленных неизотермических течений разреженного газа
    на основе асимптотической теории и проекционного метода дискретных скоростей}
...
Такое асимптотическое поведение называется \emph{эффектом призрака} (ghost effect)~\cite{Sone2002, Sone2007}.
...
например температурного и скоростного скачков, линейного термострессового скольжения (thermal-stress slip).
Это оказывается возможным, поскольку для постановки этих условий достаточно
знания решения ведущего порядка и некоторых его производных.

%%% Second order of Kn
Тепловое скольжение и нелинейная термострессовая конвекция "--- температурные течения первого порядка по \(\Kn\).
Линейные эффекты второго порядка, включая влияние кривизны граничной поверхности,
также хорошо изучены~\cite{Sone2002, Sone2007, Takata2015curvature}.
...
При малых \(k\) решение стационарное уравнения Больцмана допускает разделение пространственных масштабов,
...
\end{gather}
Здесь обозначены \(u_{iH1} = p_{H0}v_{iH1}\), \(u_{iH2} = p_{H0}v_{iH2}\) и
...
Важно отметить, что \(p_{H2}^\dag\) не входит в уравнение состояния,
...
Безразмерные транспортные коэффициенты для газа твёрдых сфер равны
\begin{alignat*}{2}\label{eq:gamma_coeffs}
    \gamma_1 &= 1.270042427, &\quad \gamma_2 &= 1.922284066, \\
    \gamma_3 &= 1.947906335, &\quad \bar{\gamma}_7 &= 1.758705.
\end{alignat*}
Первые два коэффициента соответствуют вязкости \(\mu\) и теплопроводности \(\lambda\) газа:
...
если предположить, что \(f_K\) допускает резкий перепад и убывает экспоненциально:
\begin{gather}
...
\begin{equation}\label{eq:bc_T0}
    T_{H0} = T_{B0},
\end{equation}
а значит \(f_{K0} = 0\). Используя разложения
\begin{gather}
    f_K = f_{K1}k + f_{K2}k^2 + \cdots, \label{eq:knlayer_expansion}\\
    f_H = \onwall{f_{H0}} + \left[\onwall{f_{H1}} + \eta\onwall{\pder[f_{H0}]{x_i}}n_i \right]k + \cdots, \label{eq:knlayer_exp}
\end{gather}
...
\begin{gather}
    ...
    \end{aligned}\label{eq:knlayer2}
\end{gather}

Уравнение~\eqref{eq:knlayer1} является одномерным однородным линеаризованным уравнением Больцмана
около максвеллиана \(\onwall{f_{H0}}\),
решение которого приводит к следующим граничным условиям и коррекциям макроскопических переменных:
\begin{gather}
    \frac1{\sqrt{T_{B0}}}\begin{bmatrix} (u_{jH1} - u_{Bj2}) \\ u_{jK1} \end{bmatrix} (\delta_{ij}-n_i n_j) =
        - \onwall{\pder[T_{H0}]{x_j}} (\delta_{ij}-n_i n_j)
...
Коэффициенты \(d_1\) и \(K_1\) соответствуют температурному скачку и тепловому скольжению.
...
и приводит к следующим граничным условиям и коррекциям макроскопических переменных:
\begin{gather}
    \begin{aligned}
        \frac1{\sqrt{T_{B0}}}
            \begin{bmatrix} (u_{jH2} - u_{Bj2}) \\ u_{jK2} \end{bmatrix}(\delta_{ij}-n_i n_j) =
        &- \frac{\sqrt{T_{B0}}}{p_{H0}}\onwall{\pder[u_{jH1}]{x_k}}n_k (\delta_{ij}-n_i n_j)
            \begin{bmatrix} k_0 \\ Y_0(\tilde\eta) \end{bmatrix} \\
        - \frac{T_{B0}}{p_{H0}}\onwall{\pderder[T_{H0}]{x_j}{x_k}}n_k (\delta_{ij}-n_i n_j)
            \begin{bmatrix} a_4 \\ Y_{a4}(\tilde\eta) \end{bmatrix}
        &- \bar\kappa\frac{T_{B0}}{p_{H0}}\onwall{\pder[T_{H0}]{x_j}} (\delta_{ij}-n_i n_j)
            \begin{bmatrix} a_5 \\ Y_{a5}(\tilde\eta) \end{bmatrix} \\
        - \kappa_{jk}\frac{T_{B0}}{p_{H0}}\onwall{\pder[T_{H0}]{x_k}} (\delta_{ij}-n_i n_j)
            \begin{bmatrix} a_6 \\ Y_{a6}(\tilde\eta) \end{bmatrix}
        &- \pder[T_{B1}]{x_j} (\delta_{ij}-n_i n_j)
    ...
    \begin{aligned}
        ...
        &- \frac{T_{B0}}{p_{H0}}\left[ \onwall{\pderder[T_{H0}]{x_i}{x_j}}n_i n_j
            + 2\bar\kappa\onwall{\pder[T_{H0}]{x_i}}n_i \right] n_i
...
\end{gather}
где \(\bar\kappa = (\kappa_1+\kappa_2)/2\) "--- средняя кривизна граничной поверхности,
...
Тензор кривизны \(\kappa_{ij} = \kappa_1 l_i l_j + \kappa_2 m_i m_j\)
выражается через единичные векторы соответствующих главных направлений \(l_i\) и \(m_i\).

Коэффициент \(a_4\) соответствует термострессовому скольжению.
...
\begin{equation}\label{eq:boundary_nonlinear}
    \frac1{p_{H0}^2}\onwall{\pder[T_{H0}]{x_j}\pder[T_{H0}]{x_k}}(\delta_{ij}-n_i n_j)n_k, \quad
    \frac1{p_{H0}^2}\onwall{\pder[T_{H0}]{x_i}\pder[T_{H0}]{x_j}}n_i n_j,
\end{equation}
однако решение этой неоднородной задачи кнудсеновского слоя для газа твёрдых сфер в литературе не представлено.
Для модельного уравнения Больцмана"--~Крука"--~Веландера (БКВ)
...
Множество кубатурных точек обеспечивает \(\epsilon_J < 10^{-5}\).
...
\begin{figure}
    ...
    \caption{
        Некоторые граничные интегралы в зависимости от \(\Kn\), полученные разными методами:
        уравнение теплопроводности~\ref{line1},
        уравнения КГФ с граничными условиями следующего порядка~\ref{line2} и без них~\ref{line3},
