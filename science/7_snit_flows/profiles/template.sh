...
set xlabel '$<xlabel>$' offset graph 0.5, 0.17
set ylabel '$\displaystyle <ylabel>$' offset graph <xcoord>, 0.5 rotate by 0
...
plot \
    '< paste <patch>-kes.txt <patch>-heat.txt'  using 1:($<column>-column(<column>+tc)) title 'Boltzmann equation' w l lw 2 lc 1, \
    '< paste <patch>-asym-jumps.txt <patch>-heat.txt' using 1:($<column>-column(<column>+tc)) title 'KGF with additional BC' w l lw 2 lc 2, \
