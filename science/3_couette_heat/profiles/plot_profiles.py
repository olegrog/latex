#!/usr/bin/env python
# Usage: ./plot_profiles.py /Users/olegrog/kesolver/data/ .
...
result = {}
macros = ['Pxy', 'vx', 'qx', 'qy', 'Pxx', 'Pyy', 'Pzz', 'T']
for m in macros:
    result[m] = {}
...
for kn in Kn:
    for u in U:
        ...
        #py.plot(XX, T)
        #vx, qx = asym(vx)/u/s2, asym(qx)/u/s2
        #T, Pxy, Pxx, Pyy, Pzz, qy = symm(T), symm(Pxy), symm(Pxx), symm(Pyy), symm(Pzz), symm(qy)
        result['vx'][u] = asym(vx)/u/s2
        result['qx'][u] = asym(qx)/u/s2
        result['qy'][u] = asym(qy)/u/s2
        result['Pxy'][u] = symm(Pxy)/u
        result['Pxx'][u] = symm(Pxx)/u
        result['Pyy'][u] = symm(Pyy)/u
        result['Pzz'][u] = symm(Pzz)/u
        result['T'][u] = symm(T)/u
        qy -= Pxy*vx
        if show and u>=0.1:
            Y = symm(Pxy)/u
