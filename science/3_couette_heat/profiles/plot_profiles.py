...
params = {
    'Pxy':      [ 'Pxy', 'center bottom', '-P_{xy}/\\Delta{v}', -0.2, 0.6, '-y', int(True) ],
    'vx':       [ 'vx', 'center top', 'v_x/\\Delta{v}', 0, 0.5, 'y', int(True) ],
    'qx':       [ 'qx', 'center top', '-q_x/(\\Delta{v})^2', 0, 0.35, '-y/v', int(True) ],
    'qy':       [ 'qy', 'center top', '-q_y/(\\Delta{v})^2', 0, 0.15, '-y/v', int(True) ],
    'Pxx':      [ 'Pxx', 'center bottom', '(P_{xx}-P_{yy})/(\\Delta{v})^2', -0.2, 0.5, 'y', int(False) ],
    'Pzz':      [ 'Pzz', 'center bottom', '(P_{zz}-P_{yy})/(\\Delta{v})^2', -0.01, 0.04, 'y', int(False) ],
    'tau':      [ 'tau', 'center bottom', '\\tau/(\\Delta{v})^2', 0, 0.18, 'y/v/v', int(False) ]
}
