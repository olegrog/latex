...
set ylabel '$\displaystyle\int_0^\frac12 \left( \frac{P_{yy}-P_{xx}}{(\Delta v)^2} + \frac{k}{1+2k}\right) \mathrm{d}y$' \
    offset graph 0.34, 0.48 rotate by 0
...
plot \
    ...
    "asym-1.0.txt" using 1:(base($1*k,($6-$7)/1))       notitle w l lw 2 lt 3 lc 3, \
    "asym-2.0.txt" using 1:(base($1*k,($6-$7)/2))       notitle w l lw 2 lt 4 lc 3, \
    "asym-5.0.txt" using 1:(base($1*k,($6-$7)/5))       notitle w l lw 2 lt 5 lc 3, \
    ...
    "dsmc-2.0.txt.new" using 1:(base($1*k,($6-$7)/2/2))   notitle w lp lw 2 lt 4 pt 6 lc 4, \
