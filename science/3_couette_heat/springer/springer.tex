...
\end{equation}
Note that the slip boundary conditions used in~\cite{Sharipov2000} are not correct in case of finite Mach numbers,
because they are obtained for the linear problem~\cite{Ohwada1989a}.
...
\begin{gather}
    ...
    q_x = (H_A^--H_A^+)T_{B0}\left.\pder[v_{x0}]{y}\right|_{y=\frac12}k + \frac{T_0}{p_0}\left(\frac{\gamma_3}2 T_0 \pderdual[v_{x0}]{y}
        + 4\gamma_{10} \pder[T_0]{y}\pder[v_{x0}]{y}\right)k^2 + q_{xK2}k^2 + \OO{k^3}, \label{eq:Hilbert_Qx}\\
...
the error of the projection-method solution is of the order \(10^{-4}\)
and \(10^{-5}\) for \(\Delta{v}=0.1\).
...
The large error for \(\tau\) at \(\Kn=0.01\) seems to be explained by the imperfect discretization.
The DSMC method (the green line) produce an error up to \(10^{-3}\),
