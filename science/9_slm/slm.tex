...
\title{Macroscopic model for the SLM process}
\date{\today}
\author{Oleg Rogozin}
...
\end{gather}
The initial condition is \(\Hh = 0\) and \(\Hpsi = \psi_0\).
...
Adaptive wavelet collocation method (AWCM) is employed for numerical solution of the problem
...
Evolution equation \(\pdv*{h}{t}=\mathcal{F}(h)\) is solved by the Crank--Nicolson time integration method.
...
\begin{equation}\label{eq:Drhs_diag}
    \diag{\fdv{\mathcal{F}}{h}} = a'\diag{\dder{\Hx_i}} \dder[\Hh_*]{\Hx_i} +
        a\Hh'_*\diag{\dder{\Hx_i}^2},
\end{equation}
where \(\diag{\mathcal{L}}\) is the diagonal part of finite-difference operator \(\mathcal{L}\).

Since the present problem is interface-driven, the computational mesh is refined according to
the wavelet-based local interpolation error for \(a\) and \(\Hh_*\).
...
\begin{equation}\label{eq:bc_linearized}
    \qty(b' + a\qty(1 - \psi_0) \Hh'_* \dder{\Hx_D})\var{\Hh} =
	    \frac{A(\Hpsi)\HP}{\pi^{(D-1)/2}}\exp\qty( -\qty(\Hx_i-\Hx_{Bi})^2 ) - b,
\end{equation}
where
...
\begin{equation}\label{eq:Dporosity_eval}
    \Hpsi'_0 = 0, \quad \Hpsi'_n = -\psi_0\Hpsi'_n \begin{cases}
...
\begin{gather}
    ...
    \lambda = \eval{ \frac{A(\Hpsi)\HP\exp\qty( -\qty(\Hx_i-\Hx_{Bi})^2\qty(1-\HR_p^{-2}) )}
