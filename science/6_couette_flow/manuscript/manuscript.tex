%&pdflatex
\documentclass[final]{jfm} %referee
...
The main algorithm is presented in the literature~\cite{Tcheremissine1998, Tcheremissine2006, Dodulad2015}.
We describe only the conservative technique for evaluation of the Boltzmann collision integral
on nonuniform velocity grids.
...
\end{equation}
where \(\mathcal{V} = \Set{\zeta_\gamma}{\gamma\in\Gamma}\) is a set of the integration points,
\(F\) is an arbitrary function of \(\bzeta\), \(\hat{F_\gamma}\) is used for convenient,
...
The discretization of \(\alpha_\gamma\) is assumed uniform.
...
Moreover, \eqref{eq:strict_interpolation} ensures that the H-theorem holds in the discrete form~\cite{Dodulad2013}.
For \(\bzeta_{\beta'_\nu}\) and \(\hat{f}_{\beta'_\nu}\), all formulas are similar.

The set of projection points \(\mathcal{S} = \Set{s_a}{a\in\Lambda}\)
is called the projection stencil.
In the present paper, the minimal (compact) stencil for a nonuniform velocity grid is used, \(|\mathcal{S}|=5\).
...
In addition, if the viscous boundary layer is not smooth,
the method of constructing the asymptotic solution is not obvious~\citep{Aoki2014}.

%%% Comparison between the projection method and DSMC
...
it is necessary to use a uniform velocity grid, which should be sufficiently fine to approximate
large variations of the distribution function in the Knudsen layer accurately.
This requirement can be very costly.
Stochastic techniques do not have such complications,
but at the same time has a slow convergence rate of \(\OO{N^{-1/2}}\)
(\(N\) is a total number of particles)
for both smooth and discontinuous distribution functions.
In contrast, the projection method has a satisfactory convergence of \(\OO{N^{-1}}\)
...
However, on coarser grids, the projection method can have comparable to the DSMC
performance and accuracy or sometimes better, especially for not very large \(\Kn\).
...
when significant variations of the distribution function can be localized along the axes of the velocity grid.
