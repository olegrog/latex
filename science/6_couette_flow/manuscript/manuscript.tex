...
\author{
    Oleg Rogozin\aff{1}
    \corresp{\email{oleg.rogozin@phystech.edu}}
}
\affiliation{
    \aff{1}
...
\begin{abstract}
    ...
    The nonlinear Hilbert-type asymptotic solution for a slightly rarefied gas~\citep[according to][]{Sone2000}
...
\citet{Ohwada1990} have solved it with good accuracy for hard-sphere molecules.
...
which can provide more accurate solution compared to stochastic techniques,
...
We use the obtained asymptotic solution to verify the numerical results for small Knudsen numbers.

\section{Formulation of the problem}

Consider a monatomic ideal gas confined between two infinite parallel plates with complete diffuse reflection.
...
The temperature of plates remains constant.
The \(y\) axis is normal to the plates separated by distance \(L\).

In the present paper, we deal with dimensionless quantities, hence the macroscopic variables
...
\end{equation}
where \(\ell_0\) is the mean free path for the reference density,
\(d_m\) is the diameter of the molecule.
...
therefore they were obtained by direct solution of the corresponding integral equations
...
therefore its description is omitted in the present paper,
...
in two-dimensional space \((x,y)\) under periodic boundary conditions along the \(y\) axis.
...
therefore \(\zeta_i^{(cut)}\) becomes a key factor of the accuracy.
...
There is a significant heating of the gas due to friction for large Mach numbers,
...
First one demonstrates the discrete-velocity solution of the Boltzmann equation
...
with increasing its speed (see also the asymptotic solution~\eqref{eq:eq:Hilbert_U}).
...
The asymptotic solution describes the same behaviour for \(q_x\) (see Eq.~\eqref{eq:eq:Hilbert_qx}).
...
decrease when the flow approaches to the linear case (due to interpolation~\eqref{eq:ci_interpolation}).
Moreover, linear flows require less computational cost due to smaller discrete-velocity grid
and number of excluded lattice points.
...
the main features of the Couette flow in dependence of \(\Kn\) and \(\Delta{v}\).
...
In contrast, the projection method has a satisfactory convergence \(\OO{N^{-1}}\)
(\(N\) is a total number of integration lattice points)
for a discontinuous distribution function and up to \(\OO{N^{-2}}\) for the smooth one.
...
are an order of magnitude greater than within the discrete-velocity solution even for finite \(\Delta{v}\).
