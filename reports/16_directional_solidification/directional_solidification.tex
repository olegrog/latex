...
\begin{equation}\label{eq:binary:a0}
    \Delta(a_0,k) = a_0 + K(\qS+1) - (\mathcal{G}-\beta k^2)(\qL-1 + K(\qS+1)) = 0,
\end{equation}
where
\begin{gather}
...
\begin{equation}\label{eq:bifurcation}
    ...
    \mathcal{G}_c = \beta_c k^2 + \frac{K(\pS+1)}{\mathcal{D}},
\end{equation}
...
Eq.~\eqref{eq:Gmax} can be obtained from ${\pdv*{\Delta}{\hV}}(0,k;\hV_c,\hG_c)=0$.
...
The latter case corresponds to complete solute trapping, where diffusion can be ignored.
...
Note also that Eq.~\eqref{eq:Gmax} should be replaced by
\begin{equation}\label{eq:Gmax_rapid}
    \mathcal{G}_c = 1 - \beta_ck^2\frac{\mathcal{D} + K_V'\beta_c(p_\sol+1)}{\mathcal{D} - K_V'\beta_c(p_\sol+1)},
\end{equation}
where
