...
%%% 1D geometry
Let us consider a one-dimensional compressible flow in a nozzle with a specified cross section area $A(x)$.
...
\begin{equation}\label{eq:J}
    J = J_0\exp(-\frac{16\pi}{3}\Omega^3\frac{(\crit{T}/T-1)^3}{\ln^2S}) \quad (S > 1),
\end{equation}
...
Let $A_*$, $p_*$, $T_*$, and $g_*$ be, respectively, the cross section area, pressure, temperature,
...
\begin{gather}
    \hat{J} \equiv \frac{J}{J_0}
        = \exp(-\frac{16\pi}{3}\Omega^3\frac{(\crit{T}/T_*\hat{T}-1)^3}{\ln^2S}), \label{eq:nucleation3}\\
