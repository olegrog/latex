...
\addfigure{grad13}{a Grad's distribution function~\eqref{eq:grad13} with $q_0=1$}

Evolution of a piecewise-constant distribution function in the space-homogeneous case is shown in
Figs.~\ref{fig:piecewise} and~\ref{fig:piecewise_hole}.
When the initial distribution contains negative values,
H-function can be negative and increase in time (Fig.~\ref{fig:grad13}).

We consider several cases for initial-value problem. First, we set
\begin{equation}\label{eq:piecewise}
    f(\xi_r) = \begin{cases}
        A &\text{if } -\xi_o < \xi_r < -\xi_i, \\
        Ak &\text{if } \xi_i/\sqrt{k} < \xi_r < \xi_o/\sqrt{k}.
    \end{cases}
\end{equation}
Second, we consider
\begin{equation}\label{eq:grad13}
    f(\xi_r) = f_M(\xi_r)\qty(1 + \frac{q_0}{\rho T^2}\int c\qty(\frac{c^2}{3T} - 1)\dd\xi_r).
\end{equation}
